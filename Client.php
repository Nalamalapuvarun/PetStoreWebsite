...
<html>
  ...
  <main>
    ...
    <section>
      <form name="client" action="" method="post" onsubmit="return JSValidateClient()">
    <div class="row">
      <div class="col-25">
  ...
  $con = con();
      if(isset($_POST['client']))
    {
      ...
      if($check)
      {
        ...
        $user_query = "INSERT INTO user (email,password,roleID) VALUE ('$Email','$Password',2)";
        $user_query_run = mysqli_query($con,$user_query);
        $userid_query="SELECT userid from user where email='$Email'";
        $result = mysqli_query($con,$userid_query);
        $User_id = mysqli_fetch_assoc($result);
        $user=$User_id['userid'];
        $client_query="INSERT INTO client (fname,lname,email,phone,userid) VALUES ('$Fname','$Lname','$Email','$Phone',$user)";
